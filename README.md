...
\*\*Gemini 2.5 Flash Lite over Qwen3-8B\*\* — Qwen3-8B was initially chosen for its strong Arabic comprehension, critical for a predominantly Arabic-speaking rider fleet. Migrated post-testing due to unacceptable latency on OpenRouter. Gemini delivers significantly lower TTFT on the same infrastructure while maintaining solid Arabic input handling. Both models are served via OpenRouter, keeping future model swaps to a single constant change in llm.py.
